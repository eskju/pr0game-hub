        ...
        getJSON('galaxy/' + systemCoords[1] + '/' + systemCoords[2], function (response) {
            if (response.status === 200) {
                ...
                $('content > table tr').each(function (key, obj) {
                    ...
                    if (!isNaN(planet)) {
                        $(obj).append('<td class="text-right">' + (json[planet - 1].last_battle_report || '') + '</td>');
                        $(obj).append('<td class="text-right">' + (json[planet - 1].last_spy_report || '') + '</td>');
                        $(obj).append('<td class="text-right">' + (json[planet - 1].last_spy_crystal || '') + '</td>');
                        $(obj).append('<td class="text-right">' + (json[planet - 1].last_spy_deuterium || '') + '</td>');
                        $(obj).append('<td class="text-right">' + (json[planet - 1].last_spy_deuterium || '') + '</td>');
                    }
                });
            }
