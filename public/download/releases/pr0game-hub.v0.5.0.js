...
(function() {
    ...
    window.PageOverview = function()
    {
        ...
        this.prepareHtml = function() {
            ...
            GM_setValue(ownGalaxy + ':' + ownSystem + ':' + ownPlanet + '_fieldsUsed', planetInfo.fieldsUsed ? planetInfo.fieldsUsed[1] : '');
            GM_setValue(ownGalaxy + ':' + ownSystem + ':' + ownPlanet + '_fieldsTotal', planetInfo.fieldsTotal ? planetInfo.fieldsTotal[1] : '');
            GM_setValue(ownGalaxy + ':' + ownSystem + ':' + ownPlanet + '_temperatureMin', planetInfo.temperatureMin ? planetInfo.temperatureMin[1] : '');
            GM_setValue(ownGalaxy + ':' + ownSystem + ':' + ownPlanet + '_temperatureMax', planetInfo.temperatureMax ? planetInfo.temperatureMax[1] : '');
    ...
    window.parsePageMessages = function () {
        ...
        messages.each(function (key, obj) {
            ...
            if ($(obj).find('.spyRaport').length > 0) {
                ...
                $($(messages[key - 1]).find('td')).html($($(messages[key - 1]).find('td')).html().replace(/Logbuch Nachtrag des Kommunikationsoffiziers: Dieser Bereich des Universums ist wohl noch nicht erkundet worden./, '<span style="color: ' + getRgb(cGreen) + '"><i class="fa fa-check"></i></span> Logbuch Nachtrag des Kommunikationsoffiziers: Dieser Bereich des Universums ist wohl noch nicht erkundet worden.'));
                $($(messages[key - 1]).find('td')).html($($(messages[key - 1]).find('td')).html().replace(/Logbuch Nachtrag des Kommunikationsoffiziers: Es ist ein erhebendes Gefühl, der Erste in einem unerforschten Sektor zu sein./, '<span style="color: ' + getRgb(cGreen) + '"><i class="fa fa-check"></i></span> Logbuch Nachtrag des Kommunikationsoffiziers: Es ist ein erhebendes Gefühl, der Erste in einem unerforschten Sektor zu sein.'));
                $($(messages[key - 1]).find('td')).html($($(messages[key - 1]).find('td')).html().replace(/Logbuch Nachtrag des Kommunikationsoffiziers: Es scheint nicht so, als ob jemals ein Mensch in diesem Bereich der Galaxis gewesen wäre./, '<span style="color: ' + getRgb(cRed) + '"><i class="fa fa-exclamation-triangle"></i> 25%</span> Logbuch Nachtrag des Kommunikationsoffiziers: Es scheint nicht so, als ob jemals ein Mensch in diesem Bereich der Galaxis gewesen wäre.'));
                $($(messages[key - 1]).find('td')).html($($(messages[key - 1]).find('td')).html().replace(/Logbuch Nachtrag des Kommunikationsoffiziers: Es wurden Anzeichen für die Präsenz anderer Expeditionsflotten gefunden./, '<span style="color: ' + getRgb(cRed) + '"><i class="fa fa-exclamation-triangle"></i> 50%</span> Logbuch Nachtrag des Kommunikationsoffiziers: Es wurden Anzeichen für die Präsenz anderer Expeditionsflotten gefunden.'));
                $($(messages[key - 1]).find('td')).html($($(messages[key - 1]).find('td')).html().replace(/Logbuch Nachtrag des Kommunikationsoffiziers: Wenn wir uns zu unsicher fühlen, können wir uns ja mit all den anderen Expeditionen, die hier herum fliegen, zusammen tun./, '<span style="color: ' + getRgb(cRed) + '"><i class="fa fa-exclamation-triangle"></i> 75%</span> Logbuch Nachtrag des Kommunikationsoffiziers: Wenn wir uns zu unsicher fühlen, können wir uns ja mit all den anderen Expeditionen, die hier herum fliegen, zusammen tun.'));
                ...
                if(message.match(/Wir sind auf die Überreste einer Vorgängerexpedition gestoßen! Unsere Techniker schauen, ob sie einige der Wracks wieder Flugfähig bekommen/)
                    ...
                    $($(messages[key - 1]).find('td')).html(message.replace(/([ a-zA-ZüöäÜÖÄß]+)\: ([.0-9]+)/g,'<span style="color: ' + getRgb(cRed) + ';">$1: $2</span>'));
