...
class FlightController extends Controller
{
    public function store(Request $request)
    {
        if(!$flight = Flight::query()->where('external_id', $request->get('id'))->where('is_return', $request->get('is_return'))->first()) {
            $flight = new Flight();
            $flight->external_id = $request->get('id');
        ...
        $flight->player_start_id = PlayerService::getPlayerByCoordinates($request->get('planet_start_coordinates'));
        $flight->planet_target_coordinates = $request->get('planet_target_coordinates');
        $flight->player_target_id = PlayerService::getPlayerByCoordinates($request->get('planet_target_coordinates'));
