    ...
    public function overview(Request $request): array
    {
        ...
        $query = Planet::query()
            ->select(
                ...
                #DB::raw('`players`.`score` - (SELECT MAX(`score`) FROM `log_players` WHERE `log_players`.`external_id` = `players`.`id` AND DATE(created_at) = "' . Carbon::now()->subDay()->format('Y-m-d') . '") AS `diff`')
            )
