    ...
    public function overview(Request $request): array
    {
        ...
        $query = Planet::query()
            ->select(
                ...
                DB::raw('`players`.`score` - (SELECT `score` FROM `log_players` WHERE `log_players`.`external_id` = `players`.`id` AND `log_players`.`created_at` <= "' . Carbon::now()->format('Y-m-d H:i:s') . '" ORDER BY `created_at` DESC LIMIT 1) AS `diff`')
            )
