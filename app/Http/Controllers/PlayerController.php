    ...
    public function overview(Request $request): array
    {
        ...
        $query = Planet::query()
            ->select(
                ...
                DB::raw('`players`.`score` - (SELECT `score` FROM `log_players` WHERE `log_players`.`external_id` = `players`.`id` AND DATE(created_at) = "' . Carbon::now()->subDay()->format('Y-m-d') . '" ORDER BY `created_at` LIMIT 1) AS `diff`')
            )
